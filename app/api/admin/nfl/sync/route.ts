...
import { getConfiguredProvider, getProviderStatus, requireConfiguredLiveProvider } from "@/lib/nfl-data/provider-config";
...
const BodySchema = z.object({
  ...
  op:     z.enum(["teams", "players", "schedule", "week", "live", "everything"]).default("everything")
});
...
export async function POST(request: Request) {
  try {
    ...
    const provider = op === "live" ? requireConfiguredLiveProvider() : getConfiguredProvider();

    const opType = op === "everything" ? "SYNC_EVERYTHING"
      : op === "players"  ? "SYNC_PLAYERS"
      : op === "teams"    ? "SYNC_TEAMS"
      : op === "schedule" ? "SYNC_SCHEDULE"
      : op === "live"     ? "CRON_SYNC_LIVE"
      : "SYNC_WEEK";

    const result = await runTracked(opType as Parameters<typeof runTracked>[0], async () => {
      return op === "live"
        ? syncLiveGameScores(provider, season, week)
        : syncNflData(provider, season, week);
    }, admin.id);
